# ROOT FILES CODE STACK (FRI-08-AUGUST-2025-09-48-AM)
...
#!/bin/bash
# ============================================================================
# DreamArtMachine - Robust Cron Backup Script v3.5 (Final Verified)
# Using --exclude-from for robust exclusion.
# ============================================================================

# === [ SETTINGS ] ===
BACKUP_SRC="/home/dream"
BACKUP_DEST="/home/dream/backups"
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
ARCHIVE_NAME="dream-backup-$TIMESTAMP.tar.gz"
LOGFILE="$BACKUP_DEST/backup-$TIMESTAMP.log"
EXCLUDE_FILE="/home/dream/backup_excludes.txt" # Absolute path to our exclude file

# Ensure the backup destination exists
mkdir -p "$BACKUP_DEST"

# Check if the exclude file exists before running
if [ ! -f "$EXCLUDE_FILE" ]; then
    echo "ERROR: Exclude file not found at $EXCLUDE_FILE" >> "$LOGFILE" 2>&1
    exit 1
fi

# === [ Run the Backup ] ===
# We now use --exclude-from, which is more robust than a long list of arguments.
# The verbose (-v) flag will write all actions to the log file for easy debugging.
tar -czvf "$BACKUP_DEST/$ARCHIVE_NAME" --exclude-from="$EXCLUDE_FILE" -C "$BACKUP_SRC" . > "$LOGFILE" 2>&1

# === [ Optional: Keep only the last 7 backups ] ===
# This command finds all backup files, lists them by time, skips the newest 7,
# and deletes the rest.
ls -tp "$BACKUP_DEST"/dream-backup-*.tar.gz | grep -v '/$' | tail -n +8 | xargs -r rm --
