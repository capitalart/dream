# FULL CODE STACK (FRI-08-AUGUST-2025-09-48-AM)
...
# FIX: Corrected these paths to be inside ART_PROCESSING_DIR
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function uploadFiles(files) {
    ...
    modal.classList.remove('active');
    // FIX: Changed redirect from '/upload' to '/artworks'
...
<div class="page-title-row" style="justify-content: space-between; align-items: center;">
    <h1 style="margin-bottom: 0;"><img src="{{ url_for('static', filename='icons/svg/light/number-circle-two-light.svg') }}" class="hero-step-icon" alt="Step 2: Artwork" />Artwork</h1>
    <a href="{{ url_for('artwork.upload_artwork') }}" class="btn btn-primary">Upload New Artwork</a>
</div>
<p class="page-description">Browse and analyze newly uploaded artworks. Artworks that have been processed will appear here for review.</p>
<section class="main-content">
  ...
  <div class="artwork-grid">
    ...
    <div class="gallery-card" data-analyse="{{ art.analyse or '' }}" data-aspect="{{ art.aspect }}" data-sku="{{ art.sku }}">
      ...
      <div class="card-body" style="display: flex; flex-direction: column; flex-grow: 1;">
        <h3 class="card-title" style="flex-grow: 1;">{{ art.filename }}</h3>
        ...
        <div class="button-row" style="margin-top: auto;">
            ...
            <button class="btn btn-secondary btn-analyze" data-provider="google" disabled title="Google Cloud Vision analysis coming soon!">Analyze with Google</button>
            <form method="post" action="{{ url_for('artwork.delete_artwork', slug=art.slug) }}" onsubmit="return confirm('Are you sure you want to permanently delete this artwork and all its files?');" style="width: 100%; margin: 0;">
  ...
  {% else %}
  <div style="text-align: center; padding: 3rem 1rem; background: var(--color-card-bg); border: 1px solid var(--card-border);">
    <h3>No artworks are awaiting analysis.</h3>
    <p>Upload new artwork to get started.</p>
  </div>
  {% endif %}
</section>
{% endblock %}
...
def test_processed_missing_final_json(tmp_path):
    base = tmp_path / "art-processing/processed-artwork" / "slug"
    thumbs = base / "THUMBS"
    thumbs.mkdir(parents=True)
    sku = "RJC-12345"
    slug = base.name
...
def test_processed_complete(tmp_path):
    base = tmp_path / "art-processing/processed-artwork" / "slug2"
    thumbs = base / "THUMBS"
    thumbs.mkdir(parents=True)
    sku = "R-56789"
    slug = base.name
    ...
    for i in range(1, 10):
        (base / f"{slug}-{sku}-MU-{i:02}.jpg").write_text("mu")
        (thumbs / f"{slug}-{sku}-MU-{i:02}-THUMB.jpg").write_text("mu")
    errors = check_processed(tmp_path / "art-processing/processed-artwork")
    assert errors == []
